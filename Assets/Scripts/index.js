...
function displayResults() {
    const filteredData = data.filter(item => {
        return (
            (filters.search === '' || item.name.toLowerCase().includes(filters.search) || item.description.toLowerCase().includes(filters.search) || item.hexs.some(hex => hex.includes(filters.search)) || item.cols.some(col => col.includes(filters.search)) || item.tags.some(tag => tag.includes(filters.search))) &&
